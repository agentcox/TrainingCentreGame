...
HRESULT CDisplay::CreateSurfaceFromSurface(CSurface** ppSurface, CSurface* fromSurface)
{
	DDSURFACEDESC2 ddsd;
	 HRESULT        hr;
	 *ppSurface = NULL;
    ...
    ddsd.dwFlags        = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH;
    ddsd.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN;
    ddsd.dwWidth        = fromSurface->m_ddsd.dwWidth;
    ddsd.dwHeight       = fromSurface->m_ddsd.dwHeight;
*/
	if(FAILED (fromSurface->m_pdds->GetSurfaceDesc( &ddsd ))){
		AddError("CreateSurfaceFromSurface: FAILED TO GET SOURCE DESCRIPTION");
		return E_FAIL;
	}
    ...
    {
		char crap[255];
		AddError("CreateSurfaceFromSurface: FAILED TO CREATE SURFACE FROM SURFACE DESCRIPTION");
		sprintf(crap, "^Why: Error:%d...in hex:%x", HRExtractDXErr(hr), HRExtractDXErr(hr));
		AddError(crap);
        delete (*ppSurface);
        return hr;
    }
	...
	To.left = 0;
	To.top = 0;
	To.right = ddsd.dwWidth;
	To.bottom = ddsd.dwHeight;
	if(FAILED((*ppSurface)->m_pdds->BltFast(0,0, fromSurface->m_pdds, &To, DDBLTFAST_NOCOLORKEY))){
		AddError("CreateSurfaceFromSurface: FAILED TO BLIT FROM OLD SURFACE TO NEW SURFACE");
	}
	return S_OK;
}
                               ...
                               RECT* prc )
{
	...
	//return m_pddsBackBuffer->Blt(NULL, pdds, prc, DDBLT_KEYSRC);
}
